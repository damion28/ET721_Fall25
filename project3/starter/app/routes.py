...
@main.route('/apt/fruit/<name>', methods=['GET'])
def get_fruit_by_name(name):
    """
    Get a fruit by its name from the inventory.
    """
    # Fake data for simplicity (replace with DB query if using a real DB)
    fruits = [Fruit("Apple", 10), Fruit("Banana", 5), Fruit("Orange", 7)]
    fruit = next((f for f in fruits if f.name.lower() == name.lower()), None)
    if fruit:
        return jsonify({"name": fruit.name, "quantity": fruit.quantity})
    else:
